...
//O método CheckUser é responsável por verificar se um usuário existe no banco de dados a partir de um ID que é passado na URL da requisição.
		const id = req.params.id;
		try {
			res.locals.user = userRepository.GetById(id);//ele usa o método GetById do repositório usuários para obter o usuário correspondente ao ID informado na URL.
			//caso o usuário é encontrado, ele é armazenado no objeto res.locals e a função next é chamada
			next();
		} catch (error) {
			if(error instanceof NotFoundError) return notFound(res, error.message);
			return internalError(res);
			//se ocorrer qualquer outra exceção, é retornada uma resposta de erro com o status HTTP 500 (Internal Server Error) utilizando a função internalError
		}	
	}

	static CheckEmail(req: Request, res: Response, next: NextFunction) {
//middleware que verifica se o email fornecido no corpo da solicitação já está sendo usado por outro usuário no sistema.
		try {
	...
	static CheckRequiredFields(req: Request, res: Response, next: NextFunction) {
		//responsável por verificar se todos os campos obrigatórios estão presentes na solicitação de criação de usuário e se eles possuem valores válidos.
